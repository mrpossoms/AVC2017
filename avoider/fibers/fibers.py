...
class LayerNode:
    ...
    def saver_dict(self):
        ...
        if(self.prev_layer):
            dic = self.prev_layer.saver_dict()
    ...
    def output_vector(self, size):
        ...
        weights = tf.get_variable(name='output_w_%d' % self.id, initializer=tf.truncated_normal([last(self.shape), size], stddev=0.1))
        biases  = tf.get_variable(name='output_b_%d' % self.id, initializer=tf.truncated_normal([size], stddev=0.1))

        self.parameters = [ weights, biases ]

        self.tensor = tf.add(tf.matmul(self.tensor, weights), biases)
    ...
    def to_convolution(self, filter, stride=[1, 1, 1, 1], activation_function=tf.nn.relu):
        ...
        weights = tf.get_variable(name='convolution_w_%d' % self.id, initializer=tf.truncated_normal(W, stddev=0.1))
        biases  = tf.get_variable(name='convolution_b_%d' % self.id, initializer=tf.truncated_normal([last(_filter)], stddev=0.1))
    ...
    def to_fully_connected(self, size, activation_function=tf.nn.relu):
        ...
        weights = tf.get_variable(name='fully_connected_w_%d' % self.id, initializer=tf.truncated_normal(fc_layer.shape, stddev=0.1))
        biases  = tf.get_variable(name='fully_connected_b_%d' % self.id, initializer=tf.truncated_normal([size], stddev=0.1))
